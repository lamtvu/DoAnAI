...
NodeString* BFS_search(NodeList* queueNode)
{
	...
	Init_list(daDi);
	int step = 0;
	while (queueNode->head != NULL && step < 1000)
	{
		step++;
...
#pragma endregion
	...
	do
	{
		...
		case 5:
			...
			avgDFS = (double)(end - begin) /(CLOCKS_PER_SEC*100);
			begin = clock();
			for (int i = 0; i < 100; i++)
			{
				temp = NodeRandom(n);
			...
			avgBFS = (double)(end - begin) / (CLOCKS_PER_SEC * 100);
			begin = clock();
			for (int i = 0; i < 100; i++)
			{
				temp = NodeRandom(n);
			...
			avgA = (double)(end - begin) / (CLOCKS_PER_SEC * 100);
			cout <<" "<<avgDFS <<" "<< avgBFS <<" "<< avgA;
		default:
			break;
		}
